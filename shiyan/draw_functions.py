...
class DrawFunctions:
    ...
    def get_profit_data(self, code, year):
        lg = bs.login() # 登录系统
        result_list = []
        year_list = pd.date_range(year+'1231', periods=10, freq='1Y').strftime("%Y")
        for year in tqdm(year_list): # 在这里使用tqdm显示进度条
            return_data = bs.query_profit_data(code=self.code, year=self.year, quarter=4)
        ...
        profit_data.to_csv(r'C:\Users\86138\Desktop\profit_data.csv', index=False, encoding = "GB2312")
        return profit_data

    def printresult(self):
        profit_data=self.get_profit_data(self.code, self.year)
        print(profit_data)
